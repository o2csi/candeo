...
//
// A beat is `audio.beat` (`inputs: ['audio']`): a sudden jump in the low bands,
// where a kick drum lands. The flash fades over the chosen time; between beats
// the keys keep a glow that follows the level, so quiet passages are not dark.
// The colour can turn a step around the colour wheel at each beat.
//
