...
mod tests {
    ...
    /// The keyboard's definition is, byte for byte, what its Rust family and
    /// layout gave: the same keys where they were, and the same reports for a
    /// frame, a level and each firmware effect.
    #[test]
    fn the_keyboard_definition_gives_what_the_keyboard_family_gave() {
        let (ours, _, _) = parse(KEYBOARD).unwrap();
        let theirs = &crate::ALIENWARE_M18_R1;
        assert_eq!(
        ...
        // Compared in position order: the Rust list kept a few arrows apart, and
        // everything reads keys through the matrix, never by their rank.
        let keys = |l: &Layout| {
        ...
        let bytes = |reports: Vec<Outgoing>| -> Vec<(Vec<u8>, bool)> {
            reports
                .into_iter()
                .map(|o| (o.bytes, o.wire == Wire::Feature))
                .collect()
        };
        let frame: Vec<Rgb> = (0..theirs.led_count())
            ...
            .collect();
        assert_eq!(
            bytes(ours.lighting.frame(&ours, &frame)),
            bytes(theirs.lighting.frame(theirs, &frame))
        );
        for level in [0, 0x40, 0xff] {
            assert_eq!(
                ours.lighting.brightness(level).map(bytes),
                theirs.lighting.brightness(level).map(bytes)
            );
        }
        let red = Rgb::new(255, 0, 0);
        let blue = Rgb::new(0, 0, 255);
        for id in theirs.firmware_effects.iter().map(|e| e.id).chain([OFF]) {
            for colours in [&[][..], &[red], &[red, blue]] {
                assert_eq!(
                    ours.lighting.firmware_effect(id, colours).map(|o| o.bytes),
                    theirs
                        .lighting
                        .firmware_effect(id, colours)
                        .map(|o| o.bytes),
                    "{id} {colours:?}"
                );
            }
